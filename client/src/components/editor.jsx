...
const editor = () => {
  ...
  const handleSentenceSelection = (e) => {
    const textarea = e.target; // Reference to the textarea
    const start = textarea.selectionStart;
    const end = textarea.selectionEnd;
  
    if (start !== end) {
      const selectedText = textarea.value.substring(start, end); // Get selected text
      console.log("Selected sentence:", selectedText);
  ...
  // Rephrase the selected sentence
  const rephraseSentence = async () => {
    try {
      ...
      console.log("Rephrase response:", response.data);  // Log the response
      setRephrasedSentences(response.data.rephrasedSentences);
      console.log("Rephrased sentences:", response.data.rephrasedSentences);
    } catch (error) {
  ...
  const checkSpelling = async () => {
    console.log("Text to check spelling:", text); // Check the text value
    if (!text) {
      console.error("No text provided for spell checking.");
      return;
    }
    try {
      const response = await axios.post(
        "http://localhost:8000/api/spell",
        { text },
        {
          headers: {
            Authorization: `Bearer ${await getAccessToken()}`,
      ...
      console.log("Spell checked text:", response.data.correctedText);
    } catch (error) {
      console.error("Error checking spelling:", error.response ? error.response.data : error.message);
    }
  };
  ...
  const checkGrammar = async () => {
    try {
      const response = await axios.post(
        "http://localhost:8000/api/grammar",
        { text }
      );
      setGrammarCheckedText(response.data.correctedText);
      console.log("Grammar checked text:", response.data.correctedText);
    } catch (error) {
...
{/*children represents each button */}
